...
def get_unique_filename(file_name, file_ext, extension=".txt"):
    counter = 0
    path = Path(f"{file_name}_{file_ext}_{counter}{extension}")
...
def read(sdds_name):
    ...
    file_ext = Path(input_file).suffix[1:]
    output_file = get_unique_filename(file_name, file_ext)
